...
export default function ShopSection() {
  ...
  const [inv, setInv] = useState<{ owned_item_ids: number[]; coins: number; equipped_castle: string; equipped_cannon: string } | null>(null);
    ...
    return (
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        {list.map(item => {
          ...
          const equipped = (item.item_type === "castle" ? inv.equipped_castle : inv.equipped_cannon) === item.svg_key;
          return (
            <div key={item.id}
              className={`card !p-3 flex flex-col items-center text-center gap-1.5 ${equipped ? "border-yellow-500/50" : ""}`}>
              {item.item_type === "castle" ? <CastlePreview svgKey={item.svg_key} /> : <CannonPreview svgKey={item.svg_key} />}
